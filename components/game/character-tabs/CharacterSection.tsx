...
import { ITEM_SLOT_TYPES, TU_TIEN_RANKS, VO_LAM_RANKS, FANTASY_ADVENTURER_RANKS, DEFAULT_REPUTATION_RANKS } from '../../../constants/statConstants';
...
const CoreAttributeEditor = ({ attribute, onChange }: { attribute: Stat, onChange: (id: string, value: string | number) => void }) => {
    const handleValueChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        onChange(attribute.id!, e.target.value);
    };

    let options: string[] | null = null;
    switch (attribute.name) {
        case 'Cảnh giới':
            options = TU_TIEN_RANKS;
            break;
        case 'Cấp bậc':
            options = FANTASY_ADVENTURER_RANKS;
            break;
        case 'Võ học':
            options = VO_LAM_RANKS;
            break;
        case 'Danh Vọng':
            options = DEFAULT_REPUTATION_RANKS;
            break;
    }

    return (
        <div className="core-attribute-item" title={attribute.description}>
...
export const CharacterSection = ({
    ...
    const handleCurrencyTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (value !== 'Tùy chỉnh...') {
            ...
            // When switching to custom, if custom name is empty, don't change the stat yet.
    ...
    const getTabForGenre = useCallback((genre: string) => {
        if (['Tu Tiên', 'Huyền Huyễn Truyền Thuyết'].includes(genre)) return 'Tu Tiên';
        if (['Võ Lâm', 'Thời Chiến (Trung Hoa/Nhật Bản)'].includes(genre)) return 'Võ Lâm';
        if (['Dị Giới Fantasy', 'Thế Giới Giả Tưởng (Game/Tiểu Thuyết)'].includes(genre)) return 'Dị Giới Fantasy';
        if (['Đô Thị Hiện Đại', 'Đô Thị Hiện Đại 100% bình thường', 'Đô Thị Dị Biến', 'Hậu Tận Thế'].includes(genre)) return 'Hiện Đại';
        return 'Mặc định';
    }, []);

    const activeTab = getTabForGenre(formData.genre);
    const tabs = ['Tu Tiên', 'Võ Lâm', 'Dị Giới Fantasy', 'Hiện Đại', 'Mặc định'];
    
    const handleTabClick = (tabName: string) => {
        if (tabName === activeTab) return;
    
        let newGenre = 'Trống';
        if (tabName === 'Tu Tiên') newGenre = 'Tu Tiên';
        if (tabName === 'Võ Lâm') newGenre = 'Võ Lâm';
        if (tabName === 'Dị Giới Fantasy') newGenre = 'Dị Giới Fantasy';
        if (tabName === 'Hiện Đại') newGenre = 'Đô Thị Hiện Đại';
        ...
        setFormData(prev => {
            ...
            let newTemplateIdea = prev.templateIdea;
            if (newTemplateIdea) {
                let isTemplateValid = false;
                const currentGenre = newGenre;
                ...
                if (currentGenre === 'Trống') {
                    isTemplateValid = true;
                     ...
                     for (const category in STORY_TEMPLATES) {
                        ...
                        if (template) {
                            const genreMatch = template.genres.includes(currentGenre);
            ...
            return {
                ...
                name: INITIAL_WC_FORM_DATA.name,
                // FIX: Property 'personalityOuter' does not exist on type 'WorldSettings'. Using personalityTraits instead.
                personalityTraits: INITIAL_WC_FORM_DATA.personalityTraits,
                species: INITIAL_WC_FORM_DATA.species,
                gender: INITIAL_WC_FORM_DATA.gender,
                linhCan: INITIAL_WC_FORM_DATA.linhCan,
                backstory: INITIAL_WC_FORM_DATA.backstory,
                initialRelationships: INITIAL_WC_FORM_DATA.initialRelationships,
                suggestPowerfulSkills: INITIAL_WC_FORM_DATA.suggestPowerfulSkills,
                stats: newCoreStatsTemplate,
            };
        });
    };

    const handleCoreStatChange = (statId: string, newValue: string | number) => {
        setFormData(prev => ({
            ...prev,
            stats: prev.stats.map(s => s.id === statId ? { ...s, value: newValue } : s)
        }));
    };
            ...
            <fieldset className="character-fieldset" disabled={formData.genre === 'Đồng nhân' && fanficCharOption === 'main'}>
                ...
                <div className="field-row">
                    <FormField label="Tính Cách (Bề ngoài)" htmlFor="personalityOuter">
                        {/* FIX: Changed name to 'personalityOuter' to be caught by the special handler. Value is now derived from personalityTraits. */}
                ...
                <FormField label="Sơ Lược Tiểu Sử" htmlFor="backstory"><textarea id="backstory" name="backstory" rows={3} placeholder="VD: Một phế vật mang huyết mạch thượng cổ..." value={formData.backstory} onChange={handleInputChange}></textarea></FormField>
                 ...
                 <fieldset className="skill-fieldset">
                     ...
                     <div className="core-stat-tabs">
                        {tabs.map(tabName => (
                            <button
                                type="button"
                                key={tabName}
                                className={`core-stat-tab-button ${activeTab === tabName ? 'active' : ''}`}
                    ...
                    <div className="core-attributes-grid">
                        {coreAttributes.map(attr => (
                            <CoreAttributeEditor key={attr.id} attribute={attr} onChange={handleCoreStatChange} />
                        ))}
                    </div>
                </fieldset>
